...
var MarketActions = function() {
  ...
  this.initializeMarkets = function() {
    ...
    _client.getLastMarketID( function(lastMarketID) {
      ...
      try {
        lastOpenedMarketID = web3.toDecimal(_client.getHex('LNCH', 'market'));
      }
      catch(e) {
        _client.putHex('LNCH', 'market', web3.fromDecimal(lastOpenedMarketID));
      }

      // Get favorites
      var favorites = [];
      try {
        var favs = JSON.parse(_client.getString('LNCH', 'favorites'));
        if (favs)
          favorites = favs;
      }
      catch (e) {
        _client.putString('LNCH', 'favorites', '[]');
      }
  ...
  this.switchMarket = function(market) {
    ...
    // Save last market ID
    _client.putHex('LNCH', 'market', web3.fromDecimal(market.id));
  ...
  this.toggleFavorite = function(favorite) {
    ...
    _client.putString('LNCH', 'favorites', JSON.stringify(favorites));
