...
var ConfigActions = function() {
  ...
  this.updateEthereumClient = function () {
    ...
    if (!configState.ethereumClient && ethereumClient.isAvailable()) {
      ...
      for (var key in configs) {
        ...
        try {
          result = web3.toDecimal(ethereumClient.getHex('LNCH', key));
          configs[key] = result;
        }
        catch(e) {
          ethereumClient.putHex('LNCH', key, web3.fromDecimal(configs[key]));
        }
      }
  ...
  this.updateConfig = function(payload) {
    ...
    if (payload.timeout)
      _client.putHex('LNCH', 'timeout', web3.fromDecimal(payload.timeout));

    else if (typeof payload.si !== 'undefined')
      _client.putHex('LNCH', 'si', payload.si ?
                                        web3.fromDecimal(1) :
                                        web3.fromDecimal(0));
      ...
      if (!payload.init)
        _client.putHex('LNCH', 'debug', payload.debug ? web3.fromDecimal(1) : web3.fromDecimal(0));
    }
  ...
  this.updateAddress = function(payload) {
    ...
    var _client = this.flux.stores.config.getEthereumClient();
    _client.putHex('LNCH', 'address', payload.address);
  ...
  this.updateRange = function(range) {
    ...
    var _client = this.flux.stores.config.getEthereumClient();
    _client.putHex('LNCH', 'range', web3.fromDecimal(range));
  ...
  this.updateRangeEnd = function(rangeEnd) {
    ...
    var _client = this.flux.stores.config.getEthereumClient();
    _client.putHex('LNCH', 'rangeEnd', web3.fromDecimal(rangeEnd));
