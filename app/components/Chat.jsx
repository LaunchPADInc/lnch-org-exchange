  ...
  handleSend(e) {
    ...
    if (ethereumClient.hasWhisper())
      ...
      this.setState({
        showModal: true,
        modalMessage: <p>
          TrollBox is not enabled on this node. Start <samp>geth</samp> with: <pre>--shh --rpcapi "db,eth,net,web3,shh"</pre>
        </p>
      });
    this.setState({
      message: null
    ...
    return (
      <div className={ (this.state.activeKey ? "col-xs-4" : "col-xs-2") + " chatbox"}>
        <Accordion activeKey={this.state.activeKey} onSelect={this.toggleActive}>
          <Panel header={
              <span>{ this.props.market.name } TrollBox
